...
from .serializers import OwnerRegistrationSerializer,OwnerSerializer
...
class OwnerViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if owner_name:
            queryset = queryset.filter(user__username__icontains=owner_name)
        return queryset


class OwnerRegistrationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if serialized_data.is_valid():
            ...
            confirm_link = f' http://127.0.0.1:8000/owners/active/{user_id}/{token}/'
            email_subject = 'Confirm Your Account'
            email_body = render_to_string('owner_confirmation.html', {
                'user': user,
...
class OwnerDataByUserIDView(APIView):
    ...
    def get(self, request, format=None):
        ...
        try:
            user = Owner.objects.get(user=user_id)
